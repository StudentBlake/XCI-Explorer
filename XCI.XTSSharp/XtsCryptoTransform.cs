...
namespace XCI.XTSSharp
{
    public class XtsCryptoTransform : IDisposable
    {
            ...
            int outputOffset, ulong sector)
        {
            ...
            var num = inputCount >> 4;
            var num2 = inputCount & 0xF;
            _key2.TransformBlock(_tweak, 0, _tweak.Length, _t, 0);
            var num3 = num2 != 0 ? num - 1 : num;
            ...
            else
            {
                ...
                for (; k < 16; k++) _pp[k] = _cc[k];
                TweakCrypt(_pp, 0, outputBuffer, outputOffset, _t);
            }
            return inputCount;
        }

        private static void FillArrayFromSectorLittleEndian(IList<byte> value, ulong sector)
        {
        ...
        private void TweakCrypt(IReadOnlyList<byte> inputBuffer, int inputOffset, byte[] outputBuffer, int outputOffset, byte[] t)
        {
            for (var i = 0; i < 16; i++) outputBuffer[i + outputOffset] = (byte) (inputBuffer[i + inputOffset] ^ t[i]);
        ...
        private static void MultiplyByX(IList<byte> i)
        {
            byte b = 0;
            byte b2 = 0;
            for (var j = 0; j < 16; j++)
